...
int main(int argc, char* argv[])
{
    ...
    for (const std::string& symbol : symbols) {
        ...
        std::transform(lowercase_symbol->begin(), lowercase_symbol->end(),
            lowercase_symbol->begin(), ::tolower);
    ...
    while (running) {
        ...
        for (const std::pair<const std::string, DataType::Record>& symbol : (*logbook)) {
            jobs.enqueue([&logbook, symbol = symbol.first, &db, &jobs] {
                ...
                uint64_t time = vec.back().second;

                jobs.enqueue([&db, symbol, avg, time] {
