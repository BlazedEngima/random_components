...
size_t Requester::write_callback(void* contents, size_t size, size_t nmemb,
    std::string* response)
{
...
std::vector<std::string> Requester::get_top_assets(const int limit)
{
    ...
    if (res != CURLE_OK) {
        std::cerr << "Failed to get exchange info: " << curl_easy_strerror(res)
                  << std::endl;
        return {};
    }

    nlohmann::json exchange_info = nlohmann::json::parse(response);
    std::sort(exchange_info.begin(), exchange_info.end(),
