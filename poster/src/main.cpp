...
void send_post_request(const std::string& url, const std::string& post_fields, const std::string& api_key, const std::string& api_secret) {
...
int main() {
    ...
    while (true) {
        ...
        mysqlx::Table event_queue = sess.getSchema("your_database").getTable("event_queue");
        ...
        for (const mysqlx::Row& row : res) {
            ...
            std::string post_fields = "clientOrderId=" + client_order_id + "&symbol=" + symbol + "&side=" + side + "&qty=" + qty + "&price=" + price;

            // Send a POST request to Binance Futures API
            send_post_request("https://fapi.binance.com/fapi/v1/order", post_fields, "your_api_key", "your_api_secret");

            // Delete the processed row
            event_queue.remove().where("client_order_id = :param").bind("param", client_order_id).execute();
        }
    }
